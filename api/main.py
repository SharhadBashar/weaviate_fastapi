...
async def generate_meal_plan(user_profile: UserProfile):
    try:
        prompt = f"""
        Generate a 7-day meal plan with 3 options per meal for the following user profile: Only generate dishes that are relatively common for restaurants to carry
...
async def generate_menu_prescription(user_profile: UserProfile):
    try:
        prompt = f"""
